/**
 * Send routes — create new messages.
 *
 * Maps to CLI command: inbox send
 *
 * Also exports resolveRecipients() and executeSend() for use by reply routes.
 */
...
sendRoutes.post("/", async (c) => {
  ...
  // Validate field types before calling .split() / .trim()
  if (
    body.to !== undefined &&
